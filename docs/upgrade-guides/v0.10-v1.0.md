...
# Updating from Planetmint v0.10 to v1.0

Planetmint v1.0 stands for backwards compatibility. This means that all
...
Planetmint transactions may have multiple inputs and outputs, and each output
has an amount, which is the integral number of the asset being transferred. In
prior versions of Planetmint, the amount was encoded as a number, which on the
face of it is the obvious way to encode an integer. However, as usual the devil
is in the details; JSON, the encoding of choice for Planetmint transactions,
...
The signature payload of a Planetmint transaction is now "just" the JSON
...
Crypto-Conditions specification. To send transactions to Planetmint v1.0, all
...
In order to create a correct fulfillment for an output condition in Planetmint,
...
- Usages of scheme, host and port to API V1's endpoints were removed to allow
  for configurations of Planetmint behind reverse proxies
...
- Previously this endpoint returned transactions from Planetmint's `BACKLOG`
