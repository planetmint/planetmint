...
def test_get_assets_tendermint(client, b, alice):
    ...
    res = client.get(ASSETS_ENDPOINT + "?search=" + asset)
    assert res.status_code == 200
    assert len(res.json) == 1
    assert res.json[0] == {"data": asset, "id": tx.id}
...
def test_get_assets_limit_tendermint(client, b, alice):
    ...
    res = client.get(ASSETS_ENDPOINT + "?search=" + asset1)
    assert res.status_code == 200
    assert len(res.json) == 1

    # test that only one asset is returned when using limit=1
    res = client.get(ASSETS_ENDPOINT + "?search="+asset1 + "&limit=1")
    assert res.status_code == 200
    assert len(res.json) == 1
